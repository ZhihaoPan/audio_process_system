...
def make_dataset(dir):
    ...
    if y_len > trucateLen:
        while y_len > trucateLen:
            ...
            spects.append(item)
        if y_len>0 and y_len < trucateLen:
            item = (dir, temp * trucateLen)
            spects.append(item)
    else:
        item = (dir, 0)
    ...
    return spects, y, sr, trucateLen,dur_time


def logmeldelta_loader(y, sr, begin, trucateLen, window_size, window_stride, window_type, normalize, max_len=201):
    win_length = 40
    hop_length = 220
    n_fft = int(win_length / 2000 * sr)
    n_mels_bands = 64  # num of mel features

    if begin+trucateLen<len(y):
...
def logmel_loader(y, sr, begin, trucateLen, window_size, window_stride, window_type, normalize, max_len=201):
...
def stft_loader(y, sr, begin, trucateLen, window_size, window_stride, window_type, normalize, max_len=201):
    n_fft = int(sr * window_size)
    win_length = n_fft
    hop_length = int(sr * window_stride)
    if begin+trucateLen<len(y):
...
class melLoader(data.Dataset):
    ...
    def __init__(self, dir, allLabels, transform=None, target_transform=None, window_size=.02, window_stride=.01,
                 window_type='hamming', normalize=True, max_len=201, loader_type="logmeldelta"):
        spects, self.data, self.sr, self.trucatelen, self.dur_time= make_dataset(dir)
        if len(spects) == 0:
            raise (RuntimeError(
                "Found 0 sound files in subfolders of: " + dir + "Supported audio file extensions are: " + ",".join(
                    AUDIO_EXTENSIONS)))
        self.root=dir
        self.spects = spects
