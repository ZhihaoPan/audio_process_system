...
class WorkThread4SendTempMsg(QThread):
    ...
    def __init__(self, ip4platform):
        ...
        self.socket.bind("tcp://*:5557")  # self.socket.bind("tcp://{}:5557".format(self.ip4platform))
...
class WorkThread4AudioProcess(QThread):
    ...
    def __init__(self, ID, mutex, file_path, au_cla_models, ifcuda, lang_cla_model, gpu_device, threadDict,proc_func):
    ...
    def run(self):
        try:
            ...
            au_cla_labels, dur_time, speaking_time_dict = audio_class_predict(self.file_path, self.au_cla_models,
                                                                              self.ifcuda, self.gpu_device)
            # 我系潘记号 这是你前所未见的全新版本
            ...
            self.mutex.tryLock(self.ThreadID*11)
            #mainlog("---------------{}上锁---------------------".format(file))
            time.sleep(self.ThreadID*0.3)
            try:
...
class WorkThread4LoadingModels(QThread):
    ...
    """
    trigger = pyqtSignal(dict, bool, dict, int)
    ...
    def run(self):
        try:
            ...
            lang_cla_model={}
            au_cla_models, ifcuda = loading_audio_classifier_models({"resnext": 1}, self.gpu_device)
            #lang_cla_model = loading_language_classifier_model()

            self.trigger.emit(au_cla_models, ifcuda, lang_cla_model, self.gpu_device)
        except Exception as e:
            self.mutex.lock()
